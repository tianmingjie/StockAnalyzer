...
namespace big
{
    ...
    public class BizApi
    {
        ...
        /// <returns></returns>
        public static IList<AnalyzeData> QueryAnalyzeDataWithCache(string tag, DateTime start, DateTime end, int level, string industry, string location, string type)
        {
            IEnumerable<AnalyzeData> list = new List<AnalyzeData>();
            string sql = "";
            Object o = MemcachedHelper.instance.Get("analyzedata");
            if (null != o)
            {
                list = o as List<AnalyzeData>;
            }
            else
            {
                sql = string.Format("select A.tag tag,A.level level,A.sid sid,A.name name,A.value value,A.firstlevel firstlevel,A.secondlevel secondlevel,A.enddate enddate,A.rank rank,A.startdate startdate,A.big big,B.liutonggu liutonggu, C.location location from {0} A join {6} B join {7} C on A.sid=B.sid and C.sid=A.sid and A.tag='{2}'  and A.level={3} and A.startdate='{4}' and A.enddate='{5}'  order by rank limit {1}", ANALYZE, 2800, tag, level, BizCommon.ProcessSQLString(start), BizCommon.ProcessSQLString(end), INFOEXT,INFO);
                list=BuildAnalyzeData(sql);
                MemcachedHelper.instance.Set("analyzedata", list, DateTime.Now.AddDays(1));
            }

            IList<AnalyzeData> ll = new List<AnalyzeData>();
            if (string.IsNullOrEmpty(industry) && string.IsNullOrEmpty(location))
            {
                if (!string.IsNullOrEmpty(type))
                {
                    ...
                    {
                        case "CYB":
                            //ll = list.Where(x => x.sid.StartsWith("sz3")).Select(x => x).Take(Constant.QUERY_TOP).ToList();
                            ll = (from n in list where n.sid.StartsWith("sz3") select n).Take(Constant.QUERY_TOP).ToList();
                            break;
                        case "ZXB":
                            ll = (from n in list where n.liutonggu < 3 select n).Take(Constant.QUERY_TOP).ToList();
                            break;
                        case "ZB":
                            ll = (from n in list where n.liutonggu > 20 select n).Take(Constant.QUERY_TOP).ToList();
                            break;
                        case "SH":
                            ll = (from n in list where n.sid.StartsWith("sh") select n).Take(Constant.QUERY_TOP).ToList();
                            break;

                        case "SZ":
                            ll = (from n in list where n.sid.StartsWith("sz") select n).Take(Constant.QUERY_TOP).ToList();
                            break;

                        default:
                            ll = (from n in list select n).Take(Constant.QUERY_TOP).ToList();
                            break;
            ...
            {
                if (!string.IsNullOrEmpty(industry))
                    ll = (from n in list where n.firstlevel.Equals(industry) select n).Take(Constant.QUERY_TOP).ToList();
                else if (!string.IsNullOrEmpty(location))
                    ll = (from n in list where n.location.Equals(location) select n).Take(Constant.QUERY_TOP).ToList();
            }
            return ll;
        }
        ...
        public static List<AnalyzeData> BuildAnalyzeData(string sql)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                AnalyzeData bd = new AnalyzeData()
                {
                    ...
                    liutonggu = decimal.Parse(dr["liutonggu"].ToString()),
        ...
        public static List<InfoData> QueryInfoAll()
        {
            ...
            Object o=MemcachedHelper.instance.Get("infoall");
            if(null!=o){
                list=o as List<InfoData>;
            } else{
                ...
                list=BuildInfoData(sql);
                MemcachedHelper.instance.Set("infoall", list, DateTime.Now.AddDays(7));
            }
            return list;
        }
        ...
        public static InfoData QueryInfoById(string sid)
        {
            ...
            Object o = MemcachedHelper.instance.Get("info");
            if (null != o)
            {
                id = o as InfoData;
            }
            ...
            {
                id = BuildInfoData(sql)[0];
                MemcachedHelper.instance.Set("info", id);
            }
            return id;
